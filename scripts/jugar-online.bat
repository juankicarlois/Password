@echo off
rem Lanzador del Password para jugar por internet: arranca el servidor y el tunel,
...
start "Servidor Password" /D "%ROOT%" cmd /k npm run dev

rem Un margen para que el servidor arranque antes que el tunel.
timeout /t 4 /nobreak >nul

start "Tunel Password - URL para compartir" /D "%ROOT%" cmd /k npm run tunnel
